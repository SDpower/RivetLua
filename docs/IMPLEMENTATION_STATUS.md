...
- [x] **Intermediate representation and bytecode verification:** The [compiler API](../crates/rivetlua-compiler/src/lib.rs) lowers resolved AST into typed register IR and RivetLua's own RVLU v1 bytecode. The [core verifier](../crates/rivetlua-core/src/bytecode/codec.rs) checks bounded decoding, operands, control flow, and open results before exposing a `VerifiedModule`. Both profiles pass the P05 gate. This format is not a Lua binary chunk and is not executed yet.
