...
- [x] **プロジェクトの基盤と運営文書：** Cargo ワークスペースにはコアライブラリと検証ツールがあります。MIT または Apache-2.0 のデュアルライセンスと、貢献、セキュリティ、運営に関する文書を備えています。
...
現在の値と数値のテストは Rust API を直接呼び出します。**RivetLua はまだ Lua ソースコードを読み取って実行できず、Lua 言語との完全な互換性も検証されていません。**

## 今後の作業

- [ ] Lua ソースを読み取り、字句トークンを認識します。
- [ ] 構文を解析して抽象構文木を作ります。
...
`gate P00` はプロジェクトの基盤を、`gate P01` はコアの値と数値を検査します。これらはコマンド名であり、実行時にローカルの計画書は不要です。2026-09-22 に Rust 1.98.1 で検証した結果、書式検査とワークスペースのテストが成功し、基盤の検証は 22/22、コアの検証は 35/35 が合格しました。コアの検証には基盤の回帰検査も含まれます。レポートは `target/rivetlua-reports/` に生成されます。このディレクトリは Git に含まれず、上記のコマンドで再作成できます。Cargo が宣言する MSRV は引き続き 1.94.1 です。
